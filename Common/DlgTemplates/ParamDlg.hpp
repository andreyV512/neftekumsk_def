...
template<class T, class Z>class TabPage;

template<class Table, class List>struct Insert<Table, TabPage<Table, List> >
{
	typedef TabPage<Table, List> Z;
	void operator()(Z &t, CBase &base)
	{
#pragma message("Дописать для типа TabPage<Table, List>")
		dprint("   Дописать %s\n", typeid(Z).name());
		//Insert_Into<Table>(t.table, base).Execute();
		//int id = Select<Table>(base).eq_all<Table::items_list>(&t.table.items).Execute();
		//UpdateId<ID<Table> >(base, id);
